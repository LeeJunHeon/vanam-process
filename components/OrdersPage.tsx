...
import {
  Plus, Search, Pencil, Trash2, ChevronDown, ChevronRight, CalendarDays, X, Images,
} from "lucide-react";
...
export default function OrdersPage() {
  ...
  const [viewingProc, setViewingProc] = useState<{ id: number; label: string } | null>(null);
  ...
  return (
    <div className="p-4 sm:p-6">
                      ...
                      <table className="mt-3 w-full text-sm">
                        <thead>
                          <tr className="text-left text-[11px] font-semibold text-gray-400">
                            ...
                            <th className="py-1.5 pr-3">기록</th>
                              ...
                              <td className="py-2 pr-3">
                                <button
                                  onClick={() =>
                                    setViewingProc({
                                      id: p.id,
                                      label: `${o.orderNo} #${p.sequence} ${p.processCode.code}`,
                                    })
                                  }
                                  className="flex items-center gap-1 rounded-md bg-gray-100 px-1.5 py-0.5 text-[10px] font-semibold text-gray-600 hover:bg-gray-200"
                                >
                                  <Images size={11} /> {p._count?.substrateReceipts ?? 0}
                                </button>
                              </td>
                              <td className="py-2 pr-3">
                                {p.syncStatus ? (
      ...
      {viewingProc && (
        <ReceiptListModal
          orderProcessId={viewingProc.id}
          title={viewingProc.label}
          onClose={() => setViewingProc(null)}
        />
      )}
    </div>
  );
